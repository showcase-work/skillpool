    ...
    return {
        ...
        filterOptions: function(options, term) {
            ...
            for (var i = 0; i < options.length; i++) {
                if(options[i].name){
                    label_lc = options[i].name.toLowerCase();
                    var term_lc = term.toLowerCase();
                    console.log(label_lc.indexOf(term_lc));
        ...
        for (tok in exprs) {
            if (sources && sources[tok]) {
                ...
                for (key in sources[tok]) {
                    var value = sources[tok][key].username;
                    var label = sources[tok][key].name;
    ...
    $.fn.extend({
        ...
        tagmate: function(options) {
            ...
            function replace_tag(textarea, tag, value) {
                if(tag) /* newly added to counter the bug when on click it was not working*/
                    {
            ...
            function update_menu(menu, options) {
                ...
                options = options.sort(function(a, b) {
                    var a_lc = a.name.toLowerCase();
                    var b_lc = b.name.toLowerCase();
                    if (a_lc > b_lc)
                        return 1;
                    else if (a_lc < b_lc)
                        return -1;
                ...
                for (var i = 0; i < options.length; i++) {
                    ...
                    if (image)
                        content = "<img style = \"max-width:50px; border-radius:100%; padding-right: 10px;\" src='" + image + "' alt='" + label + "'/>" + content;
                    var classes = defaults.menu_option_class;
                    if (i == 0)
                        classes += " " + defaults.menu_option_active_class;
                    menu.append("<div style = \"text-align: left; \" class='" + classes + "'>" + content + "</div>");
                }
            }
            ...
            function init_hiliter(textarea) {
                ...
                var hiliter = $("<textarea type ='text' class='tagmate-hiliter' name = 'hiddentext' id = 'hiddentext'></textarea>");
                hiliter.css("height", textarea.height() + "px");
                hiliter.css("width", textarea.width() + "px");
                hiliter.css("border", "1px solid #FFF");
                hiliter.css("display", "none");
                ...
                for (var i = 0; i < tags.length; i++) {
                    ...
                    if (sources && sources[tok]) {
                        for (var j = 0; j < sources[tok].length; j++) {
                            ...
                            console.log("working in hereeeeeeee");
                            console.log(option);
                            if (option.username == term) {
                                expr = tok + option.name;
                                break;
                            }
                        }
                    }
                    ...
                    var span = " <a href=/user/" + option.username + ">" + expr + "</a>";
            ...
            return this.each(function() {
                                ...
                                if (options && options.length > 0) {
                                    ...
                                    for (var i = 0; i < options.length; i++) {
                                        ...
                                        for (var j = 0; !found && j < _sources[tok].length; j++)
                                            found = _sources[tok][j].value == options[i].username;
                                        if (!found)
                ...
                $(document).off("click.tagmate", "." + defaults.menu_class + " ." + defaults.menu_option_class, function(){});

                $(document).on("click.tagmate", "." + defaults.menu_class + " ." + defaults.menu_option_class, function() 
                                    {
                                        var value = $(this).text();
                                        var tag = parse_tag(textarea);
